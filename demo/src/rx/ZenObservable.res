...
module Subscription = {
  type t
  @bs.send.pipe(: t) external closed: bool = "closed"
  @bs.send.pipe(: t) external unsubscribe: unit => unit = "unsubscribe"
}
...
module SubscriptionObserver = {
  ...
  @bs.send.pipe(: t<'a>) external complete: unit => unit = "complete"
}
...
@bs.send.pipe(: t<'a>)
external reduce: (('b, 'a) => 'b, 'b) => t<'b> = "reduce"

@bs.send.pipe(: t<'a>)
external flatMap: ('a => observableLike<'b>) => t<'b> = "flatMap"
